    ...
    for (qint16 indexX = 0; indexX < 8; ++indexX) {
        for (qint16 indexY = 0; indexY < 8; ++indexY) {
            ...
            if (piece.at(1) == 's') {
                ...
                //qDebug() << "Siyah : " << piece << blackTotalPoint;
                ...
                qDebug() << "Beyaz : " << piece << whiteTotalPoint;
...
{
    bool result = false;
    result = whiteBishopDanger(positionX, positionY);
    if (!result) {
        result = whiteCastleDanger(positionX, positionY);
    }
    return result;
}
